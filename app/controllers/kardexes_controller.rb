class KardexesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @kardex.save
        ...
        format.json { render action: 'show', status: :created, location: @kardex }
      else
  ...
  def destroy
    @kardex.destroy
    respond_to do |format|
      format.html { redirect_to kardexes_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def kardex_params
      params.require(:kardex).permit(:date, :detail, :income, :output, :residue)
    end
end
